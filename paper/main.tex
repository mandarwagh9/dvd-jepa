...
\begin{tikzpicture}[
  ...
  dec/.style={box,fill=gray!8,dashed},
  >=Latex
]
\node (ot) {$o_t$};
\node[enc,right=8mm of ot] (E1) {$E_\theta$};
\node[right=7mm of E1] (zt) {$z_t$};
\node[pred,right=7mm of zt] (P) {$P$};
\node[right=8mm of P] (zh) {$\hat z_{t+1}$};
\node[right=10mm of zh] (loss) {$\big\lVert \hat z_{t+1}-\mathrm{sg}(\bar z_{t+1})\big\rVert^2$};

\node[below=10mm of ot] (ot1) {$o_{t+1}$};
\node[enc,right=8mm of ot1] (E2) {$E_{\bar\theta}$};
\node[right=7mm of E2] (zb) {$\bar z_{t+1}$};

\node[dec,below=7mm of P] (D) {$D:\,z\!\to\!f$};

\draw[->] (ot) -- (E1); \draw[->] (E1) -- (zt); \draw[->] (zt) -- (P);
\draw[->] (P) -- (zh); \draw[->] (zh) -- (loss);
\draw[->] (ot1) -- (E2); \draw[->] (E2) -- (zb);
\draw[->] (zb) -| (loss);
\draw[->,dashed] (zt) -- (D);
\node[align=center,font=\scriptsize,below=1mm of E2] {EMA, stop-grad};
\node[align=center,font=\scriptsize,right=1mm of D] {optional readout};
\end{tikzpicture}
